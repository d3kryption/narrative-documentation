...
export function Tabs(columns) {
    ...
    function TabDropdown({index, row}) {
        const [activeDropdownTab, setActiveDropdownTab] = useState(activeTab);
        ...
        return (
            <li
                key={index}
                className={`tabs__item p-0 ${activeTab === row.product || row.subTabs.includes(activeTab) ? 'tabs__item--active' : ''}`}
            >
                <select
